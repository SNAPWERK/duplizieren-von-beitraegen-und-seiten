...
function duplicate_post_clone_post_link( $link = null, $before = '', $after = '', $id = 0 ) {
	...
	if ( null === $link )
	$link = esc_html__('Copy to a new draft', 'duplizieren-von-beitraegen-und-seiten');
...
function duplicate_post_admin_bar_render() {
		...
		&& (duplicate_post_is_post_type_enabled($current_object->post_type) ) )
	{
        ...
        'id' => 'new_draft',
        'title' => esc_attr__("Copy to a new draft", 'duplizieren-von-beitraegen-und-seiten'),
...
function duplicate_post_add_css() {
	...
	if ( ! empty( $current_object->post_type )
		...
		&& (duplicate_post_is_post_type_enabled($current_object->post_type) ) )
	{
		wp_enqueue_style ( 'duplizieren-von-beitraegen-und-seiten', plugins_url('/duplicate-post.css', __FILE__));
	}
}

...
function duplicate_post_show_update_notice() {
	...
	$message = '<strong><a href="https://duplicate-post.lopo.it/">'.esc_html__('Check out the new documentation for Duplicate Post!', 'duplizieren-von-beitraegen-und-seiten').'</a></strong><br/>';
	$message .= '<em>'.esc_html__('Duplicate Post is now also compatible with WPML!', 'duplizieren-von-beitraegen-und-seiten').'</em><br/>';
	$message .= '<strong>'.sprintf(wp_kses(__('Help me develop the plugin and provide support by <a href="%s">donating even a small sum</a>.', 'duplizieren-von-beitraegen-und-seiten'), array( 'a' => array( 'href' => array() ) ) ), "https://duplicate-post.lopo.it/donate").'</strong>';
	global $wp_version;
...
function duplicate_post_make_duplicate_link_row($actions, $post) {
	if (duplicate_post_is_current_user_allowed_to_copy() && duplicate_post_is_post_type_enabled($post->post_type)) {
		$actions['clone'] = '<a href="'.duplicate_post_get_clone_post_link( $post->ID , 'display', false).'" title="'
				. esc_attr__("Clone this item", 'duplizieren-von-beitraegen-und-seiten')
				. '">' .  esc_html__('Clone', 'duplizieren-von-beitraegen-und-seiten') . '</a>';
		$actions['edit_as_new_draft'] = '<a href="'. duplicate_post_get_clone_post_link( $post->ID ) .'" title="'
				. esc_attr__('Copy to a new draft', 'duplizieren-von-beitraegen-und-seiten')
				. '">' .  esc_html__('New Draft', 'duplizieren-von-beitraegen-und-seiten') . '</a>';
	}
	return $actions;
}
...
<div id="duplicate-action">
	<a class="submitduplicate duplication"
		href="<?php echo duplicate_post_get_clone_post_link( $_GET['post'] ) ?>"><?php esc_html_e('Copy to a new draft', 'duplizieren-von-beitraegen-und-seiten'); ?>
	</a>
</div>
<?php
		}
...
function duplicate_post_save_as_new_post($status = ''){
	if (! ( isset( $_GET['post']) || isset( $_POST['post'])  || ( isset($_REQUEST['action']) && 'duplicate_post_save_as_new_post' == $_REQUEST['action'] ) ) ) {
		wp_die(esc_html__('No post to duplicate has been supplied!', 'duplizieren-von-beitraegen-und-seiten'));
	}
	...
	if (isset($post) && $post!=null) {
		...
		wp_die(esc_html__('Copy creation failed, could not find original:', 'duplizieren-von-beitraegen-und-seiten') . ' ' . htmlspecialchars($id));
	}
}
...
function duplicate_post_create_duplicate($post, $status = '', $parent_id = '') {
	...
	if (!duplicate_post_is_post_type_enabled($post->post_type) && $post->post_type != 'attachment')
		wp_die(esc_html__('Copy features for this post type are not enabled in options page', 'duplizieren-von-beitraegen-und-seiten'));
...
function duplicate_post_add_plugin_links($links, $file) {
	if ( $file == plugin_basename(dirname(__FILE__).'/duplicate-post.php') ) {
        $links[] = '<a href="https://snapwerk.de">' . esc_html__('Eigene Themes &amp; Plugins entwickeln lassen', 'duplizieren-von-beitraegen-und-seiten') . '</a>';
	}
	return $links;
}
...
function duplicate_post_register_bulk_action($bulk_actions) {
	$bulk_actions['duplicate_post_clone'] = esc_html__( 'Clone', 'duplizieren-von-beitraegen-und-seiten');
	return $bulk_actions;
}
